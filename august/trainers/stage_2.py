...
def training_step(model, batch):
    ...
    m_loss = base.aml_loss()
    c_loss = c_loss / batch_size
    t_loss = t_loss / batch_size
    loss = g_loss + c_loss + t_loss*1.5 + m_loss
    return loss, g_loss, c_loss, t_loss, m_loss


def parse_args():
    parser = argparse.ArgumentParser(description="Gastric VQA Stage-2 Training with Accelerate + DeepSpeed")
    ...
    parser.add_argument("--run_name", type=str, default="gastric-vqa-stage2")
...
def main():
    ...
    dataset = SingleQADataset(args.jsonl, task="vqa")
    ...
    # wandb
    if accelerator.is_main_process:
        wandb.init(project="AUGUST", name=args.run_name, config=vars(args))
    ...
    for epoch in range(args.epochs):
        ...
        if accelerator.is_main_process:
            ...
            torch.save(unwrapped.state_dict(), f"{args.output_dir}/august_s2.pth")
